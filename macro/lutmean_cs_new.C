...
void lutmean_cs_new(TString baseFile = "1m_l3_20_0bar_180.root"){
  ...
  std::vector<TVector3> vArray[5000];
  std::vector<TVector3> lArray[5000];
  std::vector<Double_t> tArray[5000];
...
//cout<<"no problem 3 "<<endl;
      ...
      ((PrtLutNode*)(fLutNew->At(inode)))->AddEntry(node->GetDetectorId(), sum[0],pArray[j],node->GetNRefl(0),sumt,  node->GetHitPos(j), node->GetHitPosGlobal(j), node->GetDigiPos() ,vArray[j].size()/(Double_t)size,
                                                    sum[1],sum[2],sum[3],sum[4],sum[5],sum[6],sum[7],sum[8]);
