...
PrtManager::PrtManager(G4String outfile, G4int runtype){
    ...
    if(fRunType==1 || fRunType==5 || fRunType==11|| fRunType==13){ // phs
...
void PrtManager::AddHit(PrtHit hit){
    ...
    if(fRunType==1 || fRunType==5 || fRunType==11){
        if(fMomentum.Angle(fnX1) > fCriticalAngle && fMomentum.Angle(fnY1) > fCriticalAngle){
            ...
            ((PrtLutNode*)(fLut->At(id)))->AddEntry(id, fMomentum, hit.GetPathInPrizm(),hit.GetNreflectionsInPrizm(),time,hit.GetLocalPos(), hit.GetGlobalPos(), hit.GetDigiPos());
        }
    }
    
    if(fRunType==13){ // phs
        //   Double_t xy_cut = 5.0 ;
        //    if(fRunType==13 && fabs(hit.GetLocalPos().X()) < xy_cut  && fabs(hit.GetLocalPos().Y()) < xy_cut ){ // phs
        Int_t id = 100*hit.GetMcpId() + hit.GetPixelId();
        Double_t time = hit.GetLeadTime();
        TVector3 photon_direction = hit.GetMomentum();
        //std::cout<<"########### time from Maneger  "<<time <<std::endl;
        ((PrtLutNode*)(fLut->At(id)))->AddEntry(id, photon_direction, hit.GetPathInPrizm(),hit.GetNreflectionsInPrizm(),time,hit.GetLocalPos(), hit.GetGlobalPos(), hit.GetDigiPos()); // phs
    }
...
void PrtManager::FillLut(){
    if(fRunType==1 || fRunType==5 || fRunType==11 || fRunType==13) fTree->Fill(); // phs
}
