...
: G4VSensitiveDetector(name){
    ...
    /*
     ...
     hist_time_angle_ch[m][p] = new TH2F(Form("hist_time_angle_ch_%d_%d",m,p),Form("hist_time_angle_ch_%d_%d; time [ns]; solution angle [m rad]",m,p), 250,0,50,2000,0,4 );// 250 bin 400 bin
     }
     }
     */
    ...
    fAngleP = acos(sqrt(momentum*momentum+ mass[4]*mass[4])/momentum/1.4738)-0.00;
...
G4bool PrtPixelSD::ProcessHits(G4Step* step, G4TouchableHistory* hist){  
    ...
    G4ThreeVector mom_phs_Local = touchable->GetHistory()->GetTopTransform().TransformAxis(mom_phs);
    
    TVector3 mom_phs_v(mom_phs.x(),mom_phs.y(),mom_phs.z());
    TVector3 mom_phs_Local_v(mom_phs_Local.x(),mom_phs_Local.y(),mom_phs_Local.z());
    ...
    if(PrtManager::Instance()->GetRunType() == 13){ // phs
        globalPos = TVector3(globalpos.x(),globalpos.y(),globalpos.z());
    }
    ...
    if(PrtManager::Instance()->GetRunType()==13){
        ...
        mcpid=PrtManager::Instance()->GetTest1();
        pixid=PrtManager::Instance()->GetTest2();
        hit.SetMomentum(mom_phs_v);
        hit.SetMomentumLocal(mom_phs_Local_v);
    }else{
        
        mcpid=touchable->GetReplicaNumber(1);
        pixid = touchable->GetReplicaNumber(0);
        hit.SetMomentum(momentum);
        hit.SetMomentumLocal(TVector3(-1,-1,-1));
    }
    G4ThreeVector startVertix = PrtManager::Instance()->getStartVertix();
    TVector3 startVertixv(startVertix.x(),startVertix.y(),startVertix.z());
    //std::cout<<"#############  startVertix  =  "<<startVertixv.X()<<" "<<startVertixv.Y()<<" "<<startVertixv.Z()<<std::endl;
    
    hit.SetMcpId(mcpid);
    hit.SetPixelId(pixid);
    hit.SetGlobalPos(globalPos);
    hit.SetLocalPos(localPos);
    hit.SetDigiPos(startVertixv);// digiPos
    ...
    G4double time_global = step->GetPreStepPoint()->GetGlobalTime();
    G4double globalTime_track    = track->GetGlobalTime();
    
    //std::cout<<"########### time  "<<time<<" "<<time_global << " " <<globalTime_track <<std::endl;
    ...
    /*
     if(PrtManager::Instance()->GetRunType()==13)
     {
     angle = momInBar.Angle(mom_phs_v);
     
     hist_time_angle_ch[mcpid][pixid]->Fill(time,angle);
     kt = hist_time_angle_ch[mcpid][pixid]->GetXaxis()->FindBin(time);
     ka = hist_time_angle_ch[mcpid][pixid]->GetYaxis()->FindBin(angle);
     nEntries =hist_time_angle_ch[mcpid][pixid]->GetBinContent(kt,ka);
     if (nEntries>4) return false;
...
// Phs
     if(PrtManager::Instance()->GetRunType()==13){
            Double_t dir_x=mom_phs_v.X();
            Double_t dir_y=mom_phs_v.Y();
            Double_t dir_z=mom_phs_v.Z();
            if (dir_x < 0){
                if(dir_z < 0) return false;
                momInBar=TVector3(0,0,1);
            }
            if (dir_x > 0){
                if(dir_z > 0) return false;
	    ...
	    angle = momInBar.Angle(mom_phs_v);
            Double_t ang_cut= 0.020;// 0.020 0.010

            if((fAngleP-ang_cut)<angle && angle<(fAngleP+ang_cut)){

     		PrtManager::Instance()->AddHit(hit);
     		return true;
	    }
            else if((fAnglePi-ang_cut)<angle && angle<(fAnglePi+ang_cut)){
     		PrtManager::Instance()->AddHit(hit);
     		return true;
            }
            else  return false;
    ...
    if( PrtManager::Instance()->GetRunType()==13 || PrtManager::Instance()->GetRunType()==1 || PrtManager::Instance()->GetRunType()==5 || PrtManager::Instance()->GetRunType()==11 || PrtManager::Instance()->GetRunType()==6){
        PrtManager::Instance()->AddHit(hit);
        
        //std::cout<<"########### end hit  " <<std::endl;
        return true;
        //std::cout<<"########### will not appear  " <<std::endl;
    }
